...
class TwinsSVT_1d_group_SE_BP(nn.Module):
    """
    ...
    def __init__(
        ...
        # S1
        self.S1Patch = PatchEmbedding(dim=time_dim, dim_out=s1_next_dim, patch_size=s1_patch_size, group=1)
        self.S1Transformer1 = Transformer(dim=s1_next_dim, depth=1, local_patch_size=s1_local_patch_size,
                        global_k=s1_global_k, dropout=dropout, has_local=False,
                        group=time_dim, Post_norm=Post_norm)   # Convolution frames separately
                        # Transformer 1, group = 3; Transformer 2, group = 64
        self.S1PEG1 = PEG(dim=s1_next_dim, kernel_size=peg_kernel_size)
        self.S1Transformer2 = Transformer(dim=s1_next_dim, depth=1, local_patch_size=s1_local_patch_size,
                        global_k=s1_global_k, dropout=dropout, has_local=False,
                        group=1, Post_norm=Post_norm)     # Group = 1, Convolution frames together
        self.S1PEG2 = PEG(dim=s1_next_dim, kernel_size=peg_kernel_size)
        self.S1Transformer3 = Transformer(dim=s1_next_dim, depth=s1_depth, local_patch_size=s1_local_patch_size,
                        global_k=s1_global_k, dropout=dropout, has_local=True,
                        group=1, Post_norm=Post_norm)

        # S2
        self.S2Patch = PatchEmbedding(dim=s1_next_dim, dim_out=s2_next_dim, patch_size=s2_patch_size, group=1)
        self.S2Transformer1 = Transformer(dim=s2_next_dim, depth=1, local_patch_size=s2_local_patch_size,
                        global_k=s2_global_k, dropout=dropout, has_local=False,
                        group=s1_next_dim, Post_norm=Post_norm)   # Convolution frames separately
                        # Transformer 1, group = 3; Transformer 2, group = 64
        self.S2PEG1 = PEG(dim=s2_next_dim, kernel_size=peg_kernel_size)
        self.S2Transformer2 = Transformer(dim=s2_next_dim, depth=1, local_patch_size=s2_local_patch_size,
                        global_k=s2_global_k, dropout=dropout, has_local=False,
                        group=1, Post_norm=Post_norm)     # Group = 1, Convolution frames together
        self.S2PEG2 = PEG(dim=s2_next_dim, kernel_size=peg_kernel_size)
        self.S2Transformer3 = Transformer(dim=s2_next_dim, depth=s2_depth, local_patch_size=s2_local_patch_size,
                        global_k=s2_global_k, dropout=dropout, has_local=True,
                        group=1, Post_norm=Post_norm)

        self.layers = nn.Sequential(
            nn.AdaptiveAvgPool1d(1),
            Rearrange('... () -> ...'),
            nn.Linear(s2_next_dim, num_classes),
            nn.Sigmoid()
        )

    def forward(self, x):
        x = self.S1Patch(x)
        x1 = self.S1Transformer1(x)
        x1 = self.S1PEG1(x1)
        x2 = self.S1Transformer2(x1)
        x2 = self.S1PEG2(x2)
        x3 = self.S1Transformer3(x2 - x)

        x = self.S2Patch(x3 + x1)
        x1 = self.S2Transformer1(x)
        x1 = self.S2PEG1(x1)
        x2 = self.S2Transformer2(x1)
        x2 = self.S2PEG2(x2)
        x3 = self.S2Transformer3(x2 - x)
        x  = x3 + x1
...
if __name__ == '__main__':
    ...
    model = TwinsSVT_1d_group_SE_BP(num_classes=18, frames_size=3)
