...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        for lo_attn, ff1, Glo_attn, ff2, se, loGlo_attn, ff3 in self.layers:  # global_attn, ff2 in self.layers:
            a = lo_attn(x)
            a = ff1(a)
            b = Glo_attn(x)
            b = ff2(b)
...
class TwinsSVT_1d_LoGlo_SE(nn.Module):
    """
        ...
        for prefix in ('s1', 's2'):
            ...
            layers.append(nn.Sequential(  # dim = 7 dim out = 64
                ...
                Transformer(dim=next_dim, depth=1, local_patch_size=local_patch_size,
                            ...
                            group=1, Post_norm=Post_norm),   # Convolution frames separately
...
if __name__ == '__main__':
    ...
    model = TwinsSVT_1d_LoGlo_SE(num_classes=18, frames_size=3)
    model.eval()
    output = model(input)
    print(output.shape)
