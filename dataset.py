...
class SpotMatchingNoCache_SlidingWindow(Dataset):
    def __init__(self, path, features="baidu_ResNET_concat.npy", split=["train"],
                 version=2, stride=3,
                 framerate=2, window_size=3, fast_dev=False):
        self.path = path
        self.listGames = getListGames(
            split)[:5] if fast_dev else getListGames(split)
        self.features = features
        self.window_size_frame = window_size * framerate
        self.framerate = framerate
        self.split = split
        ...
        for game in tqdm(self.listGames):
            ...
            if os.path.exists(os.path.join(self.path, game, self.labels)):
                ...
                for i in range(label_half1.shape[0]):
                    # label_half1 = np.delete(label_half1, i)
                    self.all_labels.append((label_half1[i]))
                    self.save_label_position.append([game, '1_', i])
        ...
        # convert list of self.all_labels to np
    ...
    def __getitem__(self, index):
        ...
        feat = np.load(os.path.join(
            self.path, game, half + self.features), mmap_mode='r')
        feat = feat.reshape(-1, feat.shape[-1])
        idxs = np.arange(position * self.stride, position *
                         self.stride + self.window_size_frame)
        idxs = np.clip(idxs, position * self.stride, feat.shape[0] - 1)   
        feat = feat[idxs, ...]

        return feat, self.all_labels[idxs, ...].astype(np.float32)

    def __len__(self):
        return len(self.all_labels)

if __name__ == "__main__":
    da = SpotMatchingNoCache_SlidingWindow("/hdda/Datasets/SoccerNet")
    x = da[1799]
    print(x)