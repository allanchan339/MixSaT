...
class PEG(nn.Module):
    def __init__(self, dim, kernel_size=3):
        super().__init__()
        self.proj = Residual(
            nn.Conv2d(dim, dim, kernel_size=kernel_size, padding=kernel_size // 2, groups=dim, stride=1))
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads=8, dim_head=64, mlp_mult=4, local_patch_size=7, global_k=7, dropout=0.,
                 has_local=True):
        super().__init__()
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                ...
                Residual(
                    PreNorm(dim, GlobalAttention(dim, heads=heads, dim_head=dim_head, dropout=dropout, k=global_k))),
...
class TwinsSVT(nn.Module):
    def __init__(
        ...
        #     is_last = i == (num_transformer - 1)
        #
        #     dim_next = emb_dim[i]
        #
        #     layers.append(nn.Sequential(
        #         PatchEmbedding(dim=dim, dim_out=dim_next, patch_size=patch_size[i]),
        #         Transformer(dim=dim_next, depth=1, local_patch_size=local_patch_size[i],
        #                     global_k=global_k[i], dropout=dropout, has_local=not is_last),
    ...
    def forward(self, x):
        x = x[:,:,:-384]
        x = torch.reshape(x, (x.shape[0], x.shape[1], 128, 64))
        return self.layers(x)


if __name__ == '__main__':
    input = torch.randn(1, 7, 8576)
    model = TwinsSVT(num_classes=18)
    model.eval()
    output = model(input)
    print(output.shape)
