...
class Transformer(nn.Module):
    ...
    def forward(self, x):
        for lo_attn, ff1, Glo_attn, ff2, loGlo_attn, ff3 in self.layers:  # global_attn, ff2 in self.layers:
            x = lo_attn(x)
            x = ff1(x)
            x = Glo_attn(x)
            x = ff2(x)
...
class TwinsSVT_1d_group_LoGlo(nn.Module):
    """
    ...
    def __init__(
        ...
        frame_dim = frames_size  # Number of frames to input
        layers = []
        ...
        for prefix in ('s1', 's2'):
            ...
            layers.append(nn.Sequential(  # dim = 7 dim out = 64
                PatchEmbedding(dim=frame_dim, dim_out=next_dim, patch_size=patch_size, group=1),
                Transformer(dim=next_dim, depth=1, local_patch_size=local_patch_size,
                            ...
                            group=frame_dim, Post_norm=Post_norm),   # Convolution frames separately
...
if __name__ == '__main__':
    ...
    model = TwinsSVT_1d_group_LoGlo(num_classes=18, frames_size=3)
    model.eval()
    output = model(input)
    print(output.shape)
