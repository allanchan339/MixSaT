...
class PatchEmbedding(nn.Module):
    def __init__(self, *, dim, dim_out, patch_size):
        ...
        self.proj = nn.Conv1d(patch_size * dim, dim_out, 1)
...
class LocalAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., local_patch_size=7):
        super().__init__()
...
class GlobalAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0., k=7):
        ...
        self.to_q = nn.Conv1d(dim, inner_dim, 1, bias=False)
        self.to_kv = nn.Conv1d(dim, inner_dim * 2, k, stride=k, bias=False)
...
class Transformer(nn.Module):
    def __init__(self, dim, depth, heads=4, dim_head=32, mlp_mult=4, local_patch_size=7, global_k=7, dropout=0.,
                 has_local=True, Post_Norm=False):
        super().__init__()
...
class TwinsSVT_1d(nn.Module):
    ...
    def __init__(
        ...
        #         PatchEmbedding(dim=time_dim, dim_out=dim_next, patch_size=patch_size[i]),
        #         Transformer(dim=dim_next, depth=1, local_patch_size=local_patch_size[i],
        #                     global_k=global_k[i], dropout=dropout, has_local=not is_last, Post_Norm=Norm_mode),
        #         PEG(dim=dim_next, kernel_size=peg_kernel_size),
        #         Transformer(dim=dim_next, depth=depth[i], local_patch_size=local_patch_size[i],
        #                     global_k=global_k[i], dropout=dropout, has_local=not is_last, Post_Norm=Norm_mode)
        #     ))
        #
        #     time_dim = dim_next
        ...
        for prefix in ('s1', 's2'):
            ...
            layers.append(nn.Sequential(  # dim = 7 dim out = 64
                PatchEmbedding(dim=time_dim, dim_out=next_dim, patch_size=patch_size),
                Transformer(dim=next_dim, depth=1, local_patch_size=local_patch_size,
                            global_k=global_patch_size, dropout=dropout, has_local=not is_last, Post_Norm=Post_norm),
...
if __name__ == '__main__':
    input = torch.randn(1, 7, 8576)
    model = TwinsSVT_1d(num_classes=18, frames_size=7)
    model.eval()
    output = model(input)
    print(output.shape)
